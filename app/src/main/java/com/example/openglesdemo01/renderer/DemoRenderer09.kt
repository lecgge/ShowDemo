 ...
 *
 */
class DemoRenderer09(private val context: Context) : GLSurfaceView.Renderer {
    ...
    init {
        ...
        for (i in 0..50) {
            radius = 0.5f - i*0.01f
            val y = 0.5f - i*0.01f
            temp.addAll(createCircle(radius, 0f, y, 0f))
        }
    ...
    fun createCircle(radius: Float, x: Float, y: Float, z: Float): ArrayList<Float> {
        val temp: ArrayList<Float> = ArrayList()
        //添加圆心
        temp.add(x)
        temp.add(y)
        ...
        for (i in 0..52) {
            angleInRadians = ((i.toDouble()) / 52) * PI * 2
            temp.add((radius * cos(angleInRadians).toFloat()))
            temp.add(y)
            temp.add((radius * sin(angleInRadians).toFloat()))
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        Matrix.rotateM(projectionMatrix, 0, 0.5f, 1f, 0.5f, 0f)



        for (i in 0 until (tableVerticesWithTriangles.size / (6 * 54))) {
            glDrawArrays(GL_TRIANGLE_FAN, i * numPoints, numPoints)
        }
